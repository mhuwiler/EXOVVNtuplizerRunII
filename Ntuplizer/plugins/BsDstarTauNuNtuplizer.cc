    ...
    VertexDistance& distanceComputer)
{
    ...
    return std::pair<bool, Measurement1D>(true, distanceComputer.distance(VertexState(vertexPosition, vertexPositionErr), VertexState(refPoint, refPointErr)));
}




particle_cand BsDstarTauNuNtuplizer::calculateIPvariables(AnalyticalImpactPointExtrapolator extrapolator,
    RefCountedKinematicParticle particle,
    RefCountedKinematicVertex vertex,
    reco::Vertex wrtVertex)
{

    TrajectoryStateOnSurface tsos = extrapolator.extrapolate(particle->currentState().freeTrajectoryState(), RecoVertex::convertPos(wrtVertex.position()));
    ...
    particle_cand cand = {lip, lips, pvip, pvips, fl3d, fls3d, alpha};
   ...
   ********************************************************************/
    ...
    if(useDNN_)
    {
      ...
      //	mytracks.push_back(tt_track);
            ...
            pfcand _cand_ = {(Int_t)ii, (Float_t) abs(precut_dz)};
    ...
    if(isMC)
    {
        ...
        for( unsigned p=0; p < genParticles_->size(); ++p)
        {
            ...
            for(int idd = 0; idd < (int)(*genParticles_)[p].numberOfDaughters(); idd++)
            {
                ...
                if(TMath::Abs((*genParticles_)[p].daughter(idd)->pdgId())==12 ||
                    TMath::Abs((*genParticles_)[p].daughter(idd)->pdgId())==14 ||
                    TMath::Abs((*genParticles_)[p].daughter(idd)->pdgId())==16) continue;
                ...
                if(TMath::Abs((*genParticles_)[p].daughter(idd)->pdgId())==211)
                {
	               ...
	               // check matching to reco PF objects
                    ...
                    for(int kkk = 0; kkk < numOfch; kkk ++)
                    {
                        ...
                        Float_t _dR = reco::deltaR(_genvis_.Eta(), _genvis_.Phi(), _pf.eta(), _pf.phi());
            ...
            for(size_t i = 0; i < genTaus_->size(); ++ i)
            {
                ...
                visp4.SetPtEtaPhiM(visibleP4.pt(), visibleP4.eta(), visibleP4.phi(), visibleP4.mass());
	           ...
	           // calculation of the isolation
                ...
                if(isMC)
                {
                    for(unsigned int mmm=0; mmm < gps.size(); mmm++)
                    {
                        ...
                        {
                            if(reco::deltaR(tau1_fit.Eta(), tau1_fit.Phi(), tlvs[nnn].Eta(), tlvs[nnn].Phi()) < 0.015 &&
		              ...
		              //		std::cout << "\t\t\t original pt respoinse = " << (pf1.pt()/tlvs[nnn].Pt()) << ", delta_R = " << reco::deltaR(pf1.eta(), pf1.phi(), tlvs[nnn].Eta(), tlvs[nnn].Phi())  << std::endl;
                            ...
                            if(reco::deltaR(tau2_fit.Eta(), tau2_fit.Phi(), tlvs[nnn].Eta(), tlvs[nnn].Phi()) < 0.015 &&
                            ...
                            if(reco::deltaR(tau3_fit.Eta(), tau3_fit.Phi(), tlvs[nnn].Eta(), tlvs[nnn].Phi()) < 0.015 &&
...
void BsDstarTauNuNtuplizer::printout(const RefCountedKinematicVertex& myVertex)
{
  	...
  	{
    	std::cout << "\t Decay vertex: " << myVertex->position() << myVertex->chiSquared() << " " << myVertex->degreesOfFreedom() << std::endl;
  	}
  	else
  	{
